...
type
  ...
  public
    ...
    procedure polystr(x: twincontrol);// ìíîãîñòðî÷íûé çàãîëîâîê
    procedure rowada(x: string);
    procedure creax;
    function maker(y,dir: boolean): string;
...
procedure CopyStringToClipboard(const Value: String); // áóôåð äëÿ ðóññêèõ  uses ClipBrd,
const
...
var
  ...
  pData: Pointer;
begin
  Clipboard.AsText:=value;  // ðàçáëîêèðîâàòü áóôåð. õç êàê îí áëîêèðóòñÿ, ïî÷åìó îäíîçàðÿäíûé
  Clipboard.Open;
  try
...
procedure TForm1.polystr(x: twincontrol);// ìíîãîñòðî÷íûé çàãîëîâîê
var i: thandle;
begin
i:=x.handle;
...
polystr(a[high(a)]);// ìíîãîñòðî÷íûé çàãîëîâîê
end;
...
function TForm1.maker(y,dir: boolean): string;
...
//************************
if dir then   // ÷èòàòü ãîðèçîíòàëüíî
  begin
  ...
  begin  //   ÷èòàòü âåðòèêàëüíî
    s:='';
    for j:=1 to length(memo1.lines.strings[0]) do //     1  do//
      begin
        //s:=memo1.lines.strings[0];
        for i:=0 to memo1.Lines.Count-1 do
          if memo1.lines.strings[i]>'' then
            s:=s+memo1.lines.strings[i][j];//memo1.Lines.Strings[i]; ;//
      end;
  end;
